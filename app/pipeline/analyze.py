...
def analyze_audio(input_path: Path) -> Tuple[List[NoteEvent], int, List[str]]:
    ...
    try:
        # Find the bundled Basic Pitch model directory (TF SavedModel)
        model_dir = _find_basic_pitch_tf_model_dir()

        # Use one temporary workspace for decode + basic_pitch outputs, auto-cleaned.
...
def _decode_to_wav(input_path: Path, out_dir: Path, warnings: Optional[List[str]] = None) -> Path:
    """
    Decode any supported audio file (mp3/wav/flac/m4a/...) to a WAV using FFmpeg.
    Returns path to the decoded WAV file.

    NOTE: out_dir is controlled by the caller so we can clean it up reliably.
    """
    if warnings is None:
        warnings = []
    ...
    ffmpeg = _ffmpeg_exe()
    wav_path = out_dir / f"{input_path.stem}.wav"
